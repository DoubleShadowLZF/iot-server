package org.d.iot.iotserver.lock.service;
...
import org.d.iot.iotserver.lock.event.DeviceOfflineEvent;
import org.d.iot.iotserver.lock.event.DeviceOnlineEvent;
import org.d.iot.iotserver.lock.socket.message.*;
...
public class IotDoorLockManager {
  /**
   * 调用ThreadLocal前, 先调用对应的remove()方法 线程使用完毕之后,并未销毁而是等待下一次调用,这样下一次的调用有可能get到上一次set进去的内容
   * 所以调用前先remove()是正确姿势
   */
  private static Map<Integer, ChannelHandlerContext> contextMap = new ConcurrentHashMap<>();

  private static Map<ChannelHandlerContext, DeviceChannelInfo> deviceMap =
  ...
  public static void deviceRegistered(Integer id, ChannelHandlerContext cxt) {
    ...
    deviceMap.put(
        cxt, new DeviceChannelInfo().setDeviceId(id).setLastActiveTime(System.currentTimeMillis()));
    // 发送设备上线通知
  ...
  /**
   * 使用Callable+FutureTask获取执行结果 todo:是否在此添加同步锁以保证线程安全?
   *
   * @param request
   * @return
   */
   ...
   * 清理超时没有任何数据来回的设备连接
   *
   * <p>fixme: 这里可能会引起问题。比如：当清理时还有数据交互，会出问题。不过一般不会出现
   */
  public static void cleanTimeoutDevice() {
    ...
    deviceMap.forEach(
        (ctx, info) -> {
          // 检查是否超时
          if (System.currentTimeMillis() - info.getLastActiveTime()
              > serverProperties.getHeartBeatTimeout()) {
            // 超时了，关闭通道
  ...
  static class SyncRequestTask implements Callable<BaseLockMsg> {
    ...
    public BaseLockMsg call() throws Exception {
      ...
      synchronized (lock) {
        ...
        this.ctx.flush();
        log.debug(
            "request {} has sent to device, id: {}", msg.getClass().getName(), msg.getDeviceId());
